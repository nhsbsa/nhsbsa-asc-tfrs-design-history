...
After testing with processors, we found that users often missed the submissions link. Initally, we looked at the styling of the link to make it stand out or look prominent. After a discussion with the team it was clear that its also the way the previous submissions content and link was placed and nested in the explanation of what a 40 part claim is. Swapping the placement to be just before the submitter tables, allows us to match the flow in which processors start to look at a claim and gives it breathing space from the content above provides a better action point that processors are able spot as an action.
